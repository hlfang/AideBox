...
-(UIImage *)crop:(CGRect)rect scale:(CGFloat)ratio{
    CGFloat tx = rect.origin.x * ratio;
    CGFloat ty = rect.origin.y * ratio;
    CGFloat twidth = rect.size.width * ratio;
    CGFloat theight = rect.size.height * ratio;
    CGRect tRect = CGRectMake(tx, ty, twidth, theight);
    
    CGPoint origin = CGPointMake(-tx, -ty);

    UIGraphicsBeginImageContextWithOptions(tRect.size, NO, 0.0f);
...
-(UIImage *)scaleImageWithRatio:(CGFloat)ratio{
    CGSize scaleSize = CGSizeMake(self.size.width * ratio, self.size.height * ratio);
...
+(UIImage *)imageWithColor:(UIColor *)aColor{
    ...
    CGContextSetFillColorWithColor(con, aColor.CGColor);
...
-(UIImage *)cropSquare{
    ...
    if(self.size.width > self.size.height){
        tx = (self.size.width - self.size.height) * 0.5f;
        ty = 0.0f;
        twidth = theight = self.size.height;
    }else if(self.size.height > self.size.width){
        tx = 0.0f;
        ty = (self.size.height - self.size.width) * 0.5f;
        twidth = theight = self.size.width;
    }else{
        tx = ty = 0.0f;
        twidth = self.size.width;
        theight = self.size.height;
    }
    CGRect cropFrame = CGRectMake(tx, ty, twidth, theight);
    
    UIImage *resultImage = [self crop:cropFrame];
